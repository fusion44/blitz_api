...
from app.repositories.system import call_script, parse_key_value_lines, password_valid
...
async def setup_start_done(passwordA: str = ""):
    ...
    if setupPhase == "recovery":
        ...
        write_text_file(
            setupFilePath,
            [f"setupType={setupPhase}", "setPasswordA=1", f"passwordA='{passwordA}'"],
        )
...
async def setup_final_info():
    ...
    try:
        ...
        setupType = ""
    if setupType == "setup":
        return {"setupType": setupType, "seedwordsNEW": data["seedwordsNEW"]}
    else:
        return {"setupType": setupType, "seedwordsNEW": ""}
